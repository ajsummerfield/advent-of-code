...
var sample = `aaaaa-bbb-z-y-x-123[abxyz]
...
function doIt(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        var row = data[i].split('-').join('');
        var num = +(row.match(/\d+/)[0]);
        var checksum = row.match(/\[([^]+)\]/)[1];
        var letters = row.replace(num, '').replace(`[${checksum}]`, '');
        
        var orderedLettersByFrequency = countCommon(letters);
        var isRealRoom = checkTheChecksum(orderedLettersByFrequency, checksum);

        if (isRealRoom) {
            total += num;

            row = data[i].replace(/-/g, ' ');
            num = +(row.match(/\d+/)[0]);
            checksum = row.match(/\[([^]+)\]/)[1];
            letters = row.replace(num, '').replace(`[${checksum}]`, '');
            ...
            for (var j = 0; j < letters.length; j++) {
                ...
                room += String.fromCharCode((char.charCodeAt() + num - 97) % 26 + 97); // lowercase a corresponds to 97, subracting and adding it means we can go back to where we were
            }
...
function countCommon(letters) {
    var counts = {};
    letters.split('').sort().forEach((i => counts[i] = (counts[i] || 0) + 1)); // count frequency of letters

    var countArray = Object.keys(counts).map(i => [i, counts[i]]); // change to array

    return countArray.sort(function(a, b) {
        if (a[1] === b[1]) { // compare the frequency
            return a[0].charCodeAt() - b[0].charCodeAt(); // order alphabetically
        } else {
            return b[1] - a[1]; // order frequency
        }
    }).slice(0, 5).reduce((acc, cur) => acc + cur[0], '');; // take first 5 array items then only take the letters for the new array
};

function checkTheChecksum(letters, checksum) {
