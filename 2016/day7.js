...
var sample = `abba[mnop]qrst
abcd[bddb]xyyx
aaaa[qwer]tyui
ioxxoj[asdfgh]zxcvbn`;

var sample2 = `aba[bab]xyz
xyx[xyx]xyx
aaa[kek]eke
zazbz[bzb]cdb`;
...
function doIt(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        var row = data[i];
        var lettersInBrackets = row.match(/\[(.*?)\]/g);

        var invalidPairs = checkForPairs(lettersInBrackets);
        var filteredLetters = filterOutBrackets(row, lettersInBrackets);
        
        if (!invalidPairs) {
            var pairs = checkForPairs(filteredLetters);
...
function doItAgain(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        var babMatches = findMatches(lettersInBrackets);
        var abaMatches = findMatches(filteredLetters);
        var supportsSsl = hasSsl(babMatches, abaMatches);
...
function hasPairs(letters) {
    var result = false;

    for (var j = 0; j < letters.length; j++) {
        var sequence = letters.substring(j, j + 4);
        if (sequence.length === 4) {
            if (sequence[0] === sequence[1]) { // first 2 characters are the same which is invalid pairing
                result = false;
            } else {
                var pair1 = sequence[0] + sequence[1];
                var pair2 = sequence[2] + sequence[3];
    
                result = pair1 == pair2.split('').reverse().join('');
...
function filterOutBrackets(row, letters) {
    var newRow = row;

    for (var j = 0; j < letters.length; j++) {
        var sequence = letters[j];
        newRow = newRow.replace(sequence, '-');
    }

    return newRow.split('-');
};

function checkForPairs(letters) {
    var pairs = 0;
    ...
    if (letters.length) {
        ...
        for (var j = 0; j < letters.length; j++) {
            var sequence = letters[j];
            var bracketPairs = hasPairs(sequence);
...
function findMatches(letters) {
    ...
    if (letters.length) {
        ...
        for (var j = 0; j < letters.length; j++) {
            var sequence = letters[j];
            var sequenceMatches = hasMatches(sequence);
...
function hasMatches(letters) {
    var matches = [];
    
    for (var i = 0; i < letters.length; i++) {
        var sequence = letters.substring(i, i + 3);
        
        if (sequence.length === 3) {
            if (sequence[0] !== sequence[1]) { // first 2 characters are the same which is invalid pairing
                var match = sequence[0] === sequence[2];
...
function hasSsl(babMatches, abaMatches) {
    var result = false;

    for (var i = 0; i < babMatches.length; i++) {
        var bab = babMatches[i];
        
        for (var j = 0; j < abaMatches.length; j++) {
            var aba = abaMatches[j];
