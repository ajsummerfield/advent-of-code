...
function doItAgain(data) {
    ...
    for(var i = 0; i < data.length; i++) {
        var row = data[i].split(/\s+/).sort((a, b) => { return b - a });
...
function doItRewritten(data) {
    var result = data.split("\n").reduce((total, value, index, array) => total += (Math.max(...value.split(/\s+/)) - Math.min(...value.split(/\s+/))), 0);
    console.log(result);
};

function doItAgainRewritten(data) {
    var result = data.split("\n").reduce((total, value, index, array) => total += (Math.max(...value.split(/\s+/)) - Math.min(...value.split(/\s+/))), 0);
    console.log(result);
};