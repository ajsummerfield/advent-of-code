...
function doIt(data) {
    ...
    var highestEverValue = 0;

    var instructions = data.map(x => getInstruction(x));
    var registers = instructions.map(x => x.register).filter((x, index, arr) => arr.indexOf(x) == index).map(x => { return { register: x, value: 0 } });

    for (var i = 0; i < instructions.length; i++) {
        var instruction = instructions[i];
        var register = registers.filter(x => x.register === instruction.register)[0];
        var conditionRegister = registers.filter(x => instruction.registerInCondition === x.register)[0];

        var result = eval('conditionRegister.value' + instruction.condition.replace(conditionRegister.register, ''));

        if (result) {
            if (instruction.incOrDec === 'inc') {
                register.value += instruction.amount;
            } else {
                register.value -= instruction.amount;
            }

            if (register.value > highestEverValue) {
                highestEverValue = register.value;
            }
        }
    }

    largestRegister = registers.sort((a, b) => b.value - a.value)[0];

    console.log(largestRegister);
    console.log('Value: ' + highestEverValue);
};

function getInstruction(row) {
    var splitOnIf = row.split(' if ');
    var firstPart = splitOnIf[0].split(' ');
    var condition = splitOnIf[1];
    var register = firstPart[0];
    var incOrDec = firstPart[1];
    var amount = +firstPart[2];

    return {
        register: register,
        incOrDec: incOrDec,
        amount: amount,
        registerInCondition: condition.split(' ')[0],
        condition: condition,
        value: 0
    }
};