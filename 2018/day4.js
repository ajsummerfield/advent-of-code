let testInput = `[1518-11-02 00:50] wakes up
...
let input = `[1518-05-29 00:00] Guard #1151 begins shift
...
function getRecord(data) {
    ...
    if (data.includes('Guard')) {
        guardId = data.split('] ')[1].match(/\d+/g)[0];
    }

    let arr = data.split(' ');
    action = `${arr[arr.length - 2]} ${arr[arr.length - 1]}`;

    return {
        timestamp: data.split('] ')[0].substring(1),
        dateTimestamp: new Date(data.split('] ')[0].substring(1)),
        guardId: +guardId,
        action: action
    }
}
...
function printResults(obj, mostCommonMinute) {
    console.log(`Guard ID: ${obj.guardId}`);
    console.log(`Minutes asleep: ${obj.minutesAsleep}`);
    console.log(`Most common minute: ${mostCommonMinute}`);
    console.log(`Result: ${obj.guardId * mostCommonMinute}`);
    console.log('-----');
}
...
function partOne(input) {
    ...
    let sortedData = data.sort((a, b) => {
        let timestampA = new Date(a.split('] ')[0].substring(1));
        let timestampB = new Date(b.split('] ')[0].substring(1));
    ...
    for (let i = 0; i < records.length; i++) {
        ...
        if (record.action === 'wakes up') {
            let timeAsleep = Math.floor(((record.dateTimestamp - prevRecord.dateTimestamp) / 1000) / 60);
            
            if (guardSleepTime.filter(x => x.guardId == record.guardId).length) {
                let item = guardSleepTime.filter(x => x.guardId == record.guardId)[0];
                item.minutesAsleep += timeAsleep;

                for (let j = prevRecord.dateTimestamp.getMinutes(); j < (prevRecord.dateTimestamp.getMinutes() + timeAsleep); j++) {
                    item.timesFellAsleep.push(j);
                }
            } else {
                let minuteTimesAsleep = [];

                for (let j = prevRecord.dateTimestamp.getMinutes(); j < (prevRecord.dateTimestamp.getMinutes() + timeAsleep); j++) {
                    minuteTimesAsleep.push(j);
                }

                guardSleepTime.push({
                    guardId: record.guardId,
                    minutesAsleep: timeAsleep,
                    timesFellAsleep: minuteTimesAsleep
                });
            }
        }
    }
    ...
    let prevMostSleep = 0;
    let prevHighestCount = 0;

    for (let i = 0; i < guardSleepTime.length; i++) {
        let obj = guardSleepTime[i];
        let mostCommonMinute = mode(obj.timesFellAsleep);
        
        if (obj.minutesAsleep > prevMostSleep) {
            prevMostSleep = obj.minutesAsleep;
            printResults(obj, mostCommonMinute);
        }

        let count = countOccurence(obj.timesFellAsleep, mostCommonMinute);

        if (count > prevHighestCount) {
            prevHighestCount = count;
            printResults(obj, mostCommonMinute);
        }
    }
}
