...
function move(grid, location, dir, moveBy) {
...
function countSteps(grid, location, dir, moveBy) {
  var steps = 0;
...
function partOne(data) {
  ...
  for (var i = 0; i < wires.length; i++) {
    ...
    for (var j = 0; j < path.length; j++) {
      ...
      if (direction === 'R') {
        move(grid, location, dirs[0], moveBy);
      }

      if (direction === 'L') {
        move(grid, location, dirs[2], moveBy);
      }

      if (direction === 'U') {
        move(grid, location, dirs[1], moveBy);
      }

      if (direction === 'D') {
        move(grid, location, dirs[3], moveBy);
      }
    }
  }
  ...
  for (var positions in grid) {
    if (grid.hasOwnProperty(positions)) {
      if (grid[positions] === 2) {
  ...
  for (var i = 0; i < intersections.length; i++) {
    var position = intersections[i];
    var total = position.split(', ').reduce((a, b) => Math.abs(a) + Math.abs(b), 0);
    sums.push(total);
  }
...
function partTwo(data) {
  ...
  for (var i = 0; i < wires.length; i++) {
    ...
    for (var j = 0; j < path.length; j++) {
      ...
      if (direction === 'R') {
        moveInDirection = dirs[0];
      }

      if (direction === 'L') {
        moveInDirection = dirs[2];
      }

      if (direction === 'U') {
        moveInDirection = dirs[1];
      }

      if (direction === 'D') {
        moveInDirection = dirs[3];
      }

      var result = countSteps(grid, location, moveInDirection, moveBy);
      steps += result.steps;
      
      if (result.foundIntersection) {
        stepsToIntersection += steps;
        break;
      }
    }
  }

  console.log(`Total steps: ${stepsToIntersection}`);
