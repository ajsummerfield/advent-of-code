var sampleInput = `R75,D30,R83,U83,L12,D49,R71,U7,L72
...
function updateGrid(grid, x, y, moveBy, steps) {
  for (var k = 0; k < moveBy; k++) {
    grid.push({ x: x, y: y, steps: ++steps });
  }
}

function partOne(data) {
  console.log('Start ' + new Date());
  var wires = data.split('\n');
  var grids = {};

  for (var i = 0; i < wires.length; i++) {
    var path = wires[i].split(',');
    var grid = [];
    var x = 0;
    var y = 0;
    var steps = 0;
    console.log('Start Wire ' + (i + 1) + ' ' + new Date());
    
    for (var j = 0; j < path.length; j++) {
      var instruction = path[j];
      var direction = instruction[0];
      var moveBy = +instruction.substring(1);
      ...
      switch (direction) {
        case 'L':
          ...
          //updateGrid(grid, x, y, moveBy, steps);
          break;
        case 'R':
          for (var k = 0; k < moveBy; k++) {
            grid.push({ x: ++x, y: y, steps: ++steps });
    ...
    grids[`wire${i + 1}`] = grid;
    console.log('Finish Wire ' + (i + 1) + ' ' + new Date());
  }

  var intersections = [];

  console.log('Start intersections ' + new Date());
  for (var i = 0; i < grids.wire1.length; i++) {
    var wire1Coord = grids.wire1[i];

    for (var j = 0; j < grids.wire2.length; j++) {
      var wire2Coord = grids.wire2[j];

      if (wire1Coord.x === wire2Coord.x && wire1Coord.y === wire2Coord.y) {
        wire1Coord.steps += wire2Coord.steps;
        intersections.push(wire1Coord);
      }
    }
  }

  console.log('Finish intersections ' + new Date());

  console.log('Start Sums ' + new Date());
  console.log(Math.min(...intersections.map((is) => Math.abs(is.x) + Math.abs(is.y))));
  console.log('Finish Sums ' + new Date());

  var steps = intersections.map(x => x.steps);
  console.log('Steps total: ' + Math.min(...steps));
...
U7,R6,D4,L4`;
sampleInput = `R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51
